...
function CustomDrawerContent({ navigation, dispatch }) {
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      await dispatch(updateProfile({ name, email, currentPassword: password, newPassword: newPassword || undefined, photo })).unwrap();
      setEditing(false);
      setPassword('');
      setNewPassword('');
  ...
  const takePhoto = async () => {
    ...
    const result = await ImagePicker.launchCameraAsync({ allowsEditing: true, aspect: [1, 1], quality: 1, base64: true });
    if (!result.canceled) {
      setPhoto(result.assets[0].uri);
      if (!editing) dispatch(updateProfile({ photo: result.assets[0].uri, name: profile?.name, email: profile?.email }));
    }
    ...
    <View style={styles.drawerContentContainer}>
      <View style={styles.userProfileContainer}>
        <TouchableOpacity onPress={editing ? pickImage : null}>
          <Image source={{ uri: photo || 'https://randomuser.me/api/portraits/men/1.jpg' }} style={styles.userAvatar} />
          {editing && <View style={styles.cameraIcon}><Ionicons name="camera" size={20} color="white" /></View>}
        </TouchableOpacity>
        {editing ? (
          <>
            ...
            <TextInput style={styles.editInput} value={password} onChangeText={setPassword} placeholder="Current Password" secureTextEntry />
            <TextInput style={styles.editInput} value={newPassword} onChangeText={setNewPassword} placeholder="New Password (optional)" secureTextEntry />
            <View style={styles.photoButtons}>
              <TouchableOpacity style={[styles.profileButton, styles.photoButton]} onPress={pickImage} disabled={loading}><Text style={styles.buttonText}>Pick Photo</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.profileButton, styles.photoButton]} onPress={takePhoto} disabled={loading}><Text style={styles.buttonText}>Take Photo</Text></TouchableOpacity>
            </View>
          </>
        ) : (
          <>
        ...
        {editing ? (
          <View style={styles.profileActionButtons}>
            <TouchableOpacity style={[styles.profileButton, styles.saveButton, loading && styles.disabledButton]} onPress={handleUpdateProfile} disabled={loading}><Text style={styles.buttonText}>{loading ? 'Saving...' : 'Save'}</Text></TouchableOpacity>
            <TouchableOpacity style={[styles.profileButton, styles.cancelButton]} onPress={() => { setEditing(false); setName(profile?.name || ''); setEmail(profile?.email || ''); setPhoto(profile?.photo || ''); setPassword(''); setNewPassword(''); }} disabled={loading}><Text style={styles.buttonText}>Cancel</Text></TouchableOpacity>
          </View>
        ) : (
          <TouchableOpacity style={styles.profileButton} onPress={() => setEditing(true)} disabled={loading}><Text style={styles.buttonText}>Edit Profile</Text></TouchableOpacity>
        )}
      ...
      <TouchableOpacity style={styles.drawerItem} onPress={() => { navigation.navigate('Cart'); navigation.closeDrawer(); }}>
        <Ionicons name="cart" size={24} color="#6200ee" />
        <Text style={styles.drawerItemLabel}>My Cart</Text>
      </TouchableOpacity>
      <TouchableOpacity style={styles.drawerItem} onPress={() => { navigation.navigate('Orders'); navigation.closeDrawer(); }}>
...
function HomeScreenContent({ navigation }) {
  ...
  const renderBook = ({ item }) => (
    <TouchableOpacity style={styles.bookCard} onPress={() => { dispatch(fetchBookReviews(item.id)); navigation.navigate('BookDetails', { book: item }); }}>
      <Image source={{ uri: item.photo || 'https://via.placeholder.com/150' }} style={styles.bookImage} />
      <View style={styles.bookDetails}>
        ...
        <TouchableOpacity style={[styles.addToCartButton, item.stock <= 0 && styles.outOfStockButton]} onPress={() => handleAddToCart(item)} disabled={item.stock <= 0}>
          <Text style={styles.addToCartButtonText}>{item.stock <= 0 ? 'OUT OF STOCK' : 'ADD TO CART'}</Text>
        </TouchableOpacity>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.openDrawer()} style={styles.menuButton}><MaterialIcons name="menu" size={28} color="#000" /></TouchableOpacity>
        <View style={styles.searchContainer}>
          <Ionicons name="search" size={20} color="#888" style={styles.searchIcon} />
          <TextInput style={styles.searchInput} placeholder="Search Books" value={searchQuery} onChangeText={setSearchQuery} placeholderTextColor="#888" />
          <TouchableOpacity onPress={() => setShowFilters(true)} style={styles.filterButton}><Ionicons name="filter" size={20} color="#6200ee" /></TouchableOpacity>
        </View>
      </View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoriesContainer}>
        {categories.map(category => (
          <TouchableOpacity key={category} style={[styles.categoryButton, activeCategory === category && styles.activeCategoryButton]} onPress={() => setActiveCategory(category)}>
      ...
      <Modal visible={showFilters} animationType="slide" transparent={true} onRequestClose={() => setShowFilters(false)}>
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}><Text style={styles.modalTitle}>Filter Options</Text><TouchableOpacity onPress={() => setShowFilters(false)}><Ionicons name="close" size={24} color="#6200ee" /></TouchableOpacity></View>
            <View style={styles.filterSection}>
              <Text style={styles.filterLabel}>Price Range</Text>
              <View style={styles.priceRangeDisplay}><Text>${minPrice.toFixed(2)}</Text><Text> to </Text><Text>${maxPrice.toFixed(2)}</Text></View>
              <Text style={styles.sliderLabel}>Max Price</Text>
              <Slider style={styles.slider} minimumValue={0} maximumValue={maxBookPrice} minimumTrackTintColor="#6200ee" maximumTrackTintColor="#ccc" thumbTintColor="#6200ee" value={maxPrice} onValueChange={(value) => setMaxPrice(value)} step={1} />
              <Text style={styles.sliderLabel}>Min Price</Text>
              <Slider style={styles.slider} minimumValue={0} maximumValue={maxPrice} minimumTrackTintColor="#6200ee" maximumTrackTintColor="#ccc" thumbTintColor="#6200ee" value={minPrice} onValueChange={(value) => setMinPrice(value)} step={1} />
            </View>
            <View style={styles.filterButtons}>
              <TouchableOpacity style={[styles.filterButton, styles.resetButton]} onPress={resetFilters}><Text style={styles.buttonText}>Reset Filters</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.filterButton, styles.applyButton]} onPress={() => setShowFilters(false)}><Text style={styles.buttonText}>Apply Filters</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
      <View style={styles.resultsContainer}>
        <Text style={styles.resultsText}>{filteredBooks.length} {filteredBooks.length === 1 ? 'result' : 'results'} found</Text>
        {(activeCategory !== 'All' || minPrice > 0 || maxPrice < maxBookPrice || searchQuery) && <TouchableOpacity onPress={resetFilters}><Text style={styles.clearFiltersText}>Clear filters</Text></TouchableOpacity>}
      </View>
      {filteredBooks.length > 0 ? (
        <FlatList data={filteredBooks} renderItem={renderBook} keyExtractor={(item) => item.id.toString()} contentContainerStyle={styles.bookList} numColumns={2} columnWrapperStyle={styles.bookRow} />
      ) : (
        <View style={styles.noResultsContainer}>
          <Ionicons name="search" size={50} color="#ccc" />
          <Text style={styles.noResultsText}>No books found</Text>
          <Text style={styles.noResultsSubText}>Try adjusting your search or filters</Text>
          <TouchableOpacity style={styles.resetFiltersButton} onPress={resetFilters}><Text style={styles.resetFiltersButtonText}>Reset All Filters</Text></TouchableOpacity>
        </View>
      )}
    </View>
  );
...
export default function UserHomeScreen({ navigation }) {
  ...
  return (
    <>
      ...
      <Animated.View style={[styles.drawerContainer, { transform: [{ translateX: slideAnim }], width: DRAWER_WIDTH }]}>
...
const styles = StyleSheet.create({
  ...
  profileActionButtons: { flexDirection: 'row', justifyContent: 'space-between', width: '100%', marginTop: 10 },
  saveButton: { backgroundColor: '#4CAF50', width: '48%' },
  cancelButton: { backgroundColor: '#f44336', width: '48%' },
  photoButtons: { flexDirection: 'row', justifyContent: 'space-between', width: '100%', marginTop: 10, marginBottom: 10 },
